...
const string KERNEL_FILE = KERNEL_FOLDER_PATH + "hdr_operator_kernel.cl";

const string C_IMAGE_FILE = IMAGE_FOLDER_PATH + "image1_c.tga";
const string U_IMAGE_FILE = IMAGE_FOLDER_PATH + "image1_u.tga";
const string O_IMAGE_FILE = IMAGE_FOLDER_PATH + "image1_o.tga";
...
int main(int argc, char** argv)
{
	...
	RasterImage cImg = readImage(C_IMAGE_FILE.c_str());
    RasterImage uImg = readImage(U_IMAGE_FILE.c_str());
    RasterImage oImg = readImage(O_IMAGE_FILE.c_str());

	//	allocate an output image at the same dimensions
    RasterImage resultImg = newImage(cImg.width, cImg.height, cImg.type, 0);
    
    // if images have dfferent dimensions --> error and quit
	const unsigned int DATA_SIZE = cImg.height * cImg.width;
    
    cout << "image dimensions: " << cImg.width << " x " << cImg.height << endl;
    ...
    //
    cl_kernel kernel = clCreateKernel(program, "hdr_operator", &err);
    ...
    //
    cl_mem cGPUraster = clCreateBuffer(context,  CL_MEM_READ_ONLY,  sizeof(int) * DATA_SIZE, nullptr, nullptr);
    cl_mem uGPUraster = clCreateBuffer(context,  CL_MEM_READ_ONLY,  sizeof(int) * DATA_SIZE, nullptr, nullptr);
    cl_mem oGPUraster = clCreateBuffer(context,  CL_MEM_READ_ONLY,  sizeof(int) * DATA_SIZE, nullptr, nullptr);
    cl_mem outputGPUraster = clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(int) * DATA_SIZE, nullptr, nullptr);
    if (!cGPUraster || !uGPUraster || !oGPUraster || !outputGPUraster)
    {
    ...
    //
    err = clEnqueueWriteBuffer(commands, cGPUraster, CL_TRUE, 0, sizeof(int) * DATA_SIZE, cImg.raster, 0, nullptr, nullptr);
    if (err != CL_SUCCESS)
    {
        cout << "Error: Failed to write c_image to source array!" << endl;
        exit(16);
    }
    err = clEnqueueWriteBuffer(commands, uGPUraster, CL_TRUE, 0, sizeof(int) * DATA_SIZE, uImg.raster, 0, nullptr, nullptr);
    if (err != CL_SUCCESS)
    {
        cout << "Error: Failed to write u_image to source array!" << endl;
        exit(17);
    }
    err = clEnqueueWriteBuffer(commands, oGPUraster, CL_TRUE, 0, sizeof(int) * DATA_SIZE, oImg.raster, 0, nullptr, nullptr);
    if (err != CL_SUCCESS)
    {
        cout << "Error: Failed to write o_image to source array!" << endl;
        exit(18);
    }
    ...
    err = 0;
    err  = clSetKernelArg(kernel, 0, sizeof(cl_mem), &cGPUraster);
    if (err != CL_SUCCESS)
    {
        cout << "Error: Failed to set kernel argument 0! Error code " << err << endl;
        exit(19);
    }
    err  = clSetKernelArg(kernel, 1, sizeof(cl_mem), &uGPUraster);
    if (err != CL_SUCCESS)
    {
        cout << "Error: Failed to set kernel argument 1! Error code " << err << endl;
        exit(20);
    }
    err  = clSetKernelArg(kernel, 2, sizeof(cl_mem), &oGPUraster);
    if (err != CL_SUCCESS)
    {
        cout << "Error: Failed to set kernel argument 2! Error code " << err << endl;
        exit(21);
    }
    err |= clSetKernelArg(kernel, 3, sizeof(cl_mem), &outputGPUraster);
    if (err != CL_SUCCESS)
    {
        cout << "Error: Failed to set kernel argument 3! Error code " << err << endl;
        exit(22);
    }
    err |= clSetKernelArg(kernel, 4, sizeof(unsigned int), &DATA_SIZE);
    if (err != CL_SUCCESS)
    {
        cout << "Error: Failed to set kernel argument 4! Error code " << err << endl;
        exit(23);
    }
    ...
    //
    clReleaseMemObject(cGPUraster);
    clReleaseMemObject(oGPUraster);
    clReleaseMemObject(uGPUraster);
